...
Men.last <-
  data.frame(cbind(
    aggregate(
      phen.caps.stat$day_lmp_method2,
      by = list(phen.caps.stat$group_new, phen.caps.stat$Sites),
      mean,
      na.rm = TRUE
    ),
...
ggarrange(
  ...
  ggplot(
    phen.caps,
    aes(x = capsule_time, y = day_lmp_method2, color = group_new)
  )   + scale_x_continuous(breaks = c(-300, 0, 30, 60, 90, 120, 150, 180, 210)) +
    scale_y_continuous(breaks = c(0, 7, 14, 28)) +
    geom_point(
      position = position_jitterdodge(),
  ...
  ggplot(
    phen.caps,
    aes(x = group_new, y = day_lmp_method2, color = group_new)
  )  + scale_y_continuous(breaks = c(0, 7, 14, 28)) + labs(y = "", x = "") +
    geom_boxplot() + geom_point(
      position = position_jitterdodge(jitter.width = 3),
      alpha = 0.5,
...
phen.stat <- phen.va %>% mutate(vagina = 1) %>%
  mutate(gut = ifelse(KitID %in% as.vector(phen$KitID[phen$Sites == "gut"]), 1, 0)) %>%
  mutate(iw = ifelse(KitID %in% as.vector(phen$KitID[phen$Sites == "iw"]), 1, 0)) %>%
  mutate(tongue = ifelse(KitID %in% as.vector(phen$KitID[phen$Sites == "tongue"]), 1, 0))
data_graph <-
  select(phen.stat, group_by_time, SubjectID, tongue, iw, gut, vagina)
data_graph <-
  melt(
    data_graph,
...
ggplot(data_graph) +
  ...
  scale_fill_brewer(
    ...
    labels = c("Tongue coat", "Buccal mucosa", "Feces", "Vagina")
  ) +
  scale_x_continuous(
