...
dat <- dat %>%
  mutate(
    ...
    day_lmp_method2_m = ifelse(
      day_lmp_method2 <= 7,
      "S1",
      ifelse(
        day_lmp_method2 <= 14,
        "S2",
        ifelse(day_lmp_method2 <= 21, "S3", "S4")
      )
    )
  ) #NA
...
for (i in sub.list) {
  ...
  dat.draw$CVcoll <- factor(as.vector(dat.draw$CVcoll))
  dat.draw$tax <-
  ...
  ## bar plot ##
  p1 <-
    ggplot(dat.draw, aes(x = CVcoll, y = Abundance, fill = tax)) +
  ...
  fac.all <-
    c(
      ...
      levels(factor(dat$day_lmp_method2_m))
    )
  ...
  fac.note6 <- levels(factor(note.draw$day_lmp_method2_m))
  #
  ...
  p3 <-
    ggplot(note.draw, aes(x = CVcoll, y = phenotype)) +
    geom_point(
      data = note.draw,
      shape = 21,
      size = 7.5,
    ...
    geom_point(data = data.mens,      aes(color = factor(day_lmp_method2_m)), size = 4) +
    geom_point(
  ...
  ## HPV ##
  HPV.draw <-
    subset(data.hpv, SubjectID == samp.ID) %>% subset(RPKM_raw != 0) %>% subset(Sites %in% c("vagina", "gut"))
  HPV.draw <-
    merge(note.draw, HPV.draw, by = "CVcoll", all = T) %>% subset(!is.na(Sites))
  HPV.draw$Sites <- relevel(HPV.draw$Sites, ref = "vagina")
  
  p.HPV1 <-
    ggplot(subset(HPV.draw, Sites == "vagina"),
           aes(x = CVcoll, y = RPKM_raw, fill = HPV_type)) +
    geom_bar(stat = "identity", position = "stack") +
    scale_x_discrete(limits = levels(factor(note.draw$CVcoll))) +
    scale_fill_manual(
  ...
  p.HPV2 <-
    ggplot(subset(HPV.draw, Sites == "gut"),
           aes(x = CVcoll, y = RPKM_raw, fill = HPV_type)) +
    geom_bar(stat = "identity", position = "stack") +
    scale_x_discrete(limits = levels(factor(note.draw$CVcoll))) +
    scale_fill_manual(
  ...
  if (nrow(subset(HPV.draw, Sites == "vagina")) != 0) {
    if (nrow(subset(HPV.draw, Sites == "gut")) != 0) {
      ggsave(
