...
# function
source(paste0("E:\\OneDrive - BGI Tech Solutions (Hongkong) Co., Ltd\\intervene1211\\new.project\\RF\\", "rfcv1.R"))
source(paste0("E:\\OneDrive - BGI Tech Solutions (Hongkong) Co., Ltd\\intervene1211\\new.project\\RF\\", "ROC.R"))
...
##
setwd("E:/OneDrive - BGI Tech Solutions (Hongkong) Co., Ltd/intervene1211/common/GithubProbioticIntervention/")
indir <- "E:/OneDrive - BGI Tech Solutions (Hongkong) Co., Ltd/intervene1211/common/GithubProbioticIntervention/metadata/"
outdir <- "E:/OneDrive - BGI Tech Solutions (Hongkong) Co., Ltd/intervene1211/common/GithubProbioticIntervention/result/classifier/"
prefix <- paste0(outdir,"intervene.train") 
```

```{r input data for model, include=FALSE}
## 
phen <- read.csv(paste0(indir,"/phenotype.1334.vision2.csv"),header = T)%>% subset(!is.na(group_new) )
phen.va  <- subset(phen,Sites == "vagina")
###
species.pro.raw <- read.table(paste0(indir,"species.mp270.txt"),header = T)  
if (!("tax" %in% colnames(species.pro.raw))){species.pro.raw$tax <- rownames(species.pro.raw)}
rownames(species.pro.raw) <- species.pro.raw$tax ;
## species.pro filtered by 0.0001
profile.va    <- dplyr :: select(species.pro.raw,as.vector(phen.va$SeqID)) /100
profile.va.f  <- profile.va[apply(profile.va, 1, function(x){all(x<=0.0001)}),]
profile.va    <- profile.va[setdiff(rownames(profile.va),rownames(profile.va.f)),]
species.pro   <- profile.va/matrix(rep(colSums(profile.va),nrow(profile.va)),nrow(profile.va),byrow = T)
species.pro$tax <- stringr::str_split_fixed(rownames(species.pro),"s__",2)[,2]
...
bray.spe.B0 <- bray.spe %>% subset(group_new1 == "B0" & group_new2 %in% c("B1","B2"))
subsID <- as.vector((data.frame(table(bray.spe.B0$SubjectID)) %>% subset(Freq==2))$Var1)  ## SUBJECT WITHIN TWO POINTS
bray.spe.B0 <- bray.spe.B0 %>% subset(SubjectID %in% subsID)
## calculate median value by group
median.b0   <- aggregate(bray.spe.B0$value,by = list(bray.spe.B0$group_new2),median) 
## chose subjects who both lower than median
median.sub   <- dcast(bray.spe.B0,SubjectID~group_new2,value.var = "value") %>% 
  mutate(group.sub = ifelse(B2 < median.b0[median.b0$Group.1 == "B2",2] & B1< median.b0[median.b0$Group.1 == "B1",2],"low","high")) ### both lower the median
addmargins(xtabs(~group.sub,median.sub))
###
bray.spe.B0$median.b0 <- median.b0$x[match(bray.spe.B0$group_new2,median.b0$Group.1)]
bray.spe.B0$diff.median <- ifelse ((bray.spe.B0$value - bray.spe.B0$median.b0)>0,1,
                                ifelse((bray.spe.B0$value - bray.spe.B0$median.b0)<0,-1,0))
bray.spe.B0$group.sub <- median.sub$group.sub[match(bray.spe.B0$SubjectID,median.sub$SubjectID)]
...
ggplot(bray.spe,aes(x= group_new2,y= value)) + 
  ...
  geom_point(aes(color = group.sub),position = position_jitterdodge(jitter.width = 0.5),alpha = 0.5,size = 0.5)+
...
phen$group.sub <- factor(median.sub$group.sub[match(phen$SubjectID,median.sub$SubjectID)],levels = c( "low" ,"high"))
phen.base <- subset(phen, Sites == "vagina" & groups == "Baseline" & SubjectID %in% subsID)
##
data.base <-  dplyr ::select(species.pro,as.vector(phen.base$SeqID)) %>% .[rowSums(.)!=0 ,]
...
pdf(paste0(prefix,".rf_cverror.pdf" ),height = 6,width = 6)
...
par(mfrow = c(1, 2))
set.seed(123)
train.rf <- randomForest(train.x[, marker.p], train.y, ntree=500,important=TRUE,proximity=TRUE)
...
kable(imp) %>% kable_styling(full_width = F, position = "left", font_size = 16)
write.table(imp,file = paste0(prefix,".importance_class.txt"),quote = F,sep = '\t', row.names = F, col.names = T)
## save
#pdf(paste0(prefix,".Feature Species importance.pdf" ),width = 18,height = 9)
par(mfrow = c(1, 1))
varImpPlot(train.rf, main = "Top - Feature Species importance",n.var = marker.num, bg = par("bg"),color = par("fg"), gcolor = par("fg"), lcolor = "gray" )
#dev.off()
```
...
phen.va.436 <- read.csv(paste0(indir,"/phenotype.1334.vision2.csv"),header = T)%>% 
  subset(Sites == "vagina" )
###
pret.profile <- data.frame(t(dplyr :: select(species.pro.raw,as.vector(phen.va.436$SeqID))/100) ) %>% .[,colSums(.)!=0]
colnames(pret.profile) <- stringr::str_split_fixed(colnames(pret.profile),"s__",2)[,2]
##
...
###
pret.res <- data.frame( prob = pret.p[, 2]) %>% rownames_to_column("SeqID")
pret.res$group.pret <- ifelse(pret.res$prob >0.5 ,"high" ,"low")
write.table(pret.res, paste0(prefix,"_pret_probability_all_436.group.txt"), sep = "\t", quote = F, col.names = T,row.names = F)
